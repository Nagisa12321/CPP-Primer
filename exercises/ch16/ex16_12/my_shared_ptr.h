...
public:
    ...
    T *operator->() const;
    operator bool() const;
    template <typename _Base> operator my_shared_ptr<_Base>();

    long use_count() const;
    bool unique() const;
    void reset(T *new_pt = 0);
...
my_shared_ptr<T>::operator my_shared_ptr<_Base>() {
    my_shared_ptr<_Base> res;
    return res;
}
